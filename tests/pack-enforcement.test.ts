...
describe("blocked_tags enforcement", () => {
  ...
  before(async () => {
    ...
    const { createBehaviorPack, setActivePack } = await import("../src/kb/vault.js");
...
describe("exportPackClosure", () => {
  ...
  it("exports pack + pinned cards into a bundle", async () => {
    ...
    const result = await exportPackClosure();
    ...
    const packJson = await fs.readFile(
      path.join(result.bundle_path, "pack.json"),
      "utf-8"
    );
